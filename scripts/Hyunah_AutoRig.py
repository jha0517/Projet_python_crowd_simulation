...
def Controllers():
    ...
    for i in range(len(side)):
        cmds.ikHandle(n= charName+'Leg'+side[i]+'ikHandle', sj= charName+side[i]+'thigh_Jnt_01', ee=charName+side[i]+'ankie_Jnt_01')
        cmds.spaceLocator(n=charName+'poleVector'+side[i]+'leg',p=(nb[i]*locXYZ[4][0]-legY*0.005,locXYZ[4][1]+legY*0.05,locXYZ[4][2]))
        cmds.xform(centerPivots=1)
        # aims the pole vector of 1 at 2.
        cmds.poleVectorConstraint( charName+'poleVector'+side[i]+'leg', charName+'Leg'+side[i]+'ikHandle' )
        cmds.move(nb[i]*lengthY*0.75,-lengthY*0.75,charName+'poleVector'+side[i]+'leg',moveXY=True)
        cmds.setAttr(charName+'Leg'+side[i]+'ikHandle.twist',nb[i]*90)
        cmds.ParentConstraint('controllerfoot',charName+'poleVector'+side[i]+'leg')
        cmds.parent( charName+'poleVector'+side[i]+'leg', charName+'Leg'+side[i]+'ikHandle', relative=True )
        ...
        cmds.group(charName+'Leg'+side[i]+'ikHandle', n= charName+'Foot'+side[i]+'heelPeel')
        ...
        cmds.parent( charName+'Foot'+side[i]+'heelPeel','Foot'+side[i]+'TipToe', 'Foot'+side[i]+'1', relative=True )
        cmds.move(Xpos, Ypos, Zpos, 'Foot'+side[i]+'1.scalePivot','Foot'+side[i]+'1.rotatePivot', absolute=True)
        Xpos = cmds.getAttr('Foot'+side[i]+'ball_ikHandle.translateX' )
        Ypos = cmds.getAttr('Foot'+side[i]+'ball_ikHandle.translateY' )
        Zpos = cmds.getAttr('Foot'+side[i]+'ball_ikHandle.translateZ' )
        CreateCtr('Foot'+side[i]+'Crl','Foot'+side[i]+'1',(Xpos,Ypos,Zpos),(lengthY/60*10,lengthY/60*10,lengthY/60*16),(0,0,0))
    #left Arm
    for i in range(len(side)):
        ...
        #left Arm controller
        CreateCtr('Arm'+side[i]+'Crl','Arm'+side[i]+'ikHandle',(nb[i]*locXYZ[3][0],locXYZ[3][1],locXYZ[3][2]),(lengthY/60*5,lengthY/60*5,lengthY/60*8),(0,0,nb[i]*30))
    #spline
...
cmds.button( label='Walking cycle')
cmds.setParent( '..' )
cmds.setParent( '..' )
